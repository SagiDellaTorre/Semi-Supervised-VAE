...
class DecoderModule(nn.Module):
    def __init__(self, input_channels, output_channels, stride, pad, activation="relu"):
        super().__init__()
        self.convt = nn.ConvTranspose2d(input_channels, output_channels, kernel_size=stride, padding=pad, stride=stride)
        self.bn = nn.BatchNorm2d(output_channels)
        if activation == "relu":
...
class Decoder(nn.Module):
    def __init__(self, color_channels, pooling_kernels, decoder_input_size):
        ...
        super().__init__()
        self.m1 = DecoderModule(256, 128 , stride=1, pad=0)
        self.m2 = DecoderModule(128, 64, stride=pooling_kernels[1], pad=0)
        self.m3 = DecoderModule(64, 32, stride=pooling_kernels[0], pad=2)
        self.bottle = DecoderModule(32, color_channels, stride=1, pad=0, activation="sigmoid")
...
class M2(object):
    ...
    """
    def __init__(self, latent_features, classifier, lr=1.0e-3, cuda=True, dataset = "mnist", unlabeled_proportion = 15, path="m2.pth"):
        self.model = M2_base(latent_features,classifier)
        self.device = torch.device("cuda" if cuda and torch.cuda.is_available() else "cpu")
        self.model.to(self.device)
        self.lr = lr
        self.dataset = dataset
        self.unlabeled_proportion = str(unlabeled_proportion)
        self.path = path
        self.initialize
    ...
    def fit(self, train_loader,test_loader, epochs,alpha,labeled_data_len):
        ...
        plt.xlabel('epochs') 
        plt.ylabel('ELBO') 
        plt.title(f'Dataset: {self.dataset}')
        plt.savefig(f'{plots_dir_name}/{self.dataset}_unlabeled_prop_{self.unlabeled_proportion}_loss.jpg')     
        ...
        plt.ylabel('accuracy') 
        plt.title(f'Dataset: {self.dataset}')
        plt.savefig(f'{plots_dir_name}/{self.dataset}_unlabeled_prop_{self.unlabeled_proportion}_accuracy.jpg')           
    ...
    def _train(self,train_loader,test_loader,optimizer,epoch,alpha,labeled_data_len):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if not os.path.exists(samples_dir_name):
                os.makedirs(samples_dir_name)            
            
            if batch_idx == len(train_loader)-1:
                if ( sum(labels != 10) > 0):
                        save_image(recon_labeled, f"{samples_dir_name}/{self.dataset}_unlabeled_prop_{self.unlabeled_proportion}_reconstruction_epoch_{str(epoch)}.png", nrow=8)
                        save_image(labeled_data, f"{samples_dir_name}/{self.dataset}_unlabeled_prop_{self.unlabeled_proportion}_truth_epoch_{str(epoch)}.png", nrow=8)
...
class Classifier(nn.Module):
    ...
    def __init__(self, image_reso, filter_size, dropout_rate):
        ...
        # color_channels input channels, 10 output channels, 5 x 5 filter size
        self.conv1_drop = nn.Dropout2d(dropout_rate)
        self.conv1 = nn.Conv2d(color_channels, 10, kernel_size=filter_size, stride = 1, padding = (filter_size - 1)//2)  
    ...
    def forward(self, x):
        ...
        # color_channels x 96 x 96 -> 10 x 96 x 96 
        x = self.conv1_drop(x)
        x = self.conv1(x)
        x = self.bn1(x)
        x = F.relu(x)
